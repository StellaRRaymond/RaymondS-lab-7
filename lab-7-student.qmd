...
fish|>
  ...
  group_by(year)|>
  mutate(weight_na = sum(is.na(weight)
                         )
         )|>
  ...
  geom_line()+
  theme_bw()+
  labs(title = "Missing fish weights over time",
       x = "Time (yrs)",
       y ="",
...
You used length within the rescaling for weight! As a result, the values will be compeltely messed up because you are no longer using the minimum value for weight but instead for length. Grams and cm do not translate as cooperable units.
...
rescale_01 <- function(x){return((x - min(x, na.rm = TRUE)) / 
