...
fish |>
  ...
  mutate(condition = condition_fish(wei = weight, len = length),
         mean_con = mean(condition))|>
  ungroup()|>
  ggplot(mapping = aes(x = year, y = mean_con, color = species)) +
  geom_point(alpha = .5)+
  labs(title = "Change in Condition Index of Blackfoot River Fish",
       x = "Time (Years)",
       y = "",
       subtitle = "Y-Axis Representing Fish Condition")
```

*Reflection: In the end, there was one small yet influential mistake I made. It was not in fact the order of operations as stated but instead how i coded the function. Instead of removing the maximum and minimum values, i make it such that any outliers were made into the minimum and maximum. This, as a result, did not *
