...
condition_fish <- function(len, wei){
  (wei * 100)/(len^3)}
...
fish |>
  mutate(condition = condition_fish(length, weight))|>
  ggplot(mapping = aes(x = year, y = condition, color = species)) +
  geom_jitter()+
  labs(title = "Change in Condition Index of ",
       x = "Time (Years)",
       y = "",
       subtitle = "Y-Axis Representing Fish Condition ((weight*100)/(length^3))")
